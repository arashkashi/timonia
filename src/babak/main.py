...
coins_original = { 'coin_id' : ['curve-dao-token'] , 'symbol' : ['CRV'] } #  
...
"""
    ...
    # plt.plot(df['total_volume'].head(6).values)
    ...
    klines['date'] = pd.to_datetime(klines['timestamp'], unit='ms')
    df['date'] = pd.to_datetime(df['timestamp'], unit='ms')
    
    df = pd.merge(df, klines , on=['date'])
    
    ## the line right below generated some error and has to be commented out.
    #df = df.astype(float)
