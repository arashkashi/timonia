...
for coin in coins.index : #### len(coins['coin_id'])
    ...
    # print(df['success_long'].value_counts())
    # print(a)

    df['long_condition'] = np.where((df.Heiken_color.shift(1) == 'red') & (df.Heiken_color.shift(2) == 'red') & (df.Heiken_color.shift(3) == 'red') & (df.Un_Norm_Vol_MCap) & (df.Greater) & (df.Close.shift(1) <df.Open.shift(1)) & (df.Close.shift(2) <df.Open.shift(2)) , True , False) # & (df.Close.shift(3) >df.Open.shift(3)) (df.Un_Norm_Vol_MCap.shift(1) != True) &
    df['long_next_candle_success'] = np.where((df.long_condition) & (df.Close > df.Open), True , False)
    df['long_two_candle_success'] =  np.where((df.long_next_candle_success) & (df.Close.shift(-1) > df.Open.shift(-1)), True , np.nan)

    for i in range(len(df.index)) :
        if df.long_condition[df.index[i]] :
    ...
    df_results.at['long_condition',coin] =df['long_condition'].sum()
    df_results.at['long_next_candle_success',coin] =df['long_next_candle_success'].sum()
    df_results.at['long_two_candle_success',coin] =df['long_two_candle_success'].sum()
...
df_results.to_csv('results\\result_3Heiken_2last red_long_percent_unprevious.csv')
...
print('non_common_day : ' , df_results.sum()['long_condition']-commonday)
